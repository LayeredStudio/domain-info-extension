	...
	data() {
		return {
			...
			whoisGroup: [
				{
					name: 'General info',
					fields: [ 'Domain Name', 'IDN', 'Domain Status', 'Name Server', 'Created Date', 'Updated Date', 'Expiry Date', 'ROID', 'Registry Domain ID', 'Domain ID', 'DNSSEC' ],
				},
				{
					name: 'Registrar',
					fields: [ 'Registrar', 'Sponsoring Registrar', 'Record maintained by', 'Registrar URL', 'Registrar WHOIS Server', 'Registry WHOIS Server', 'Registrar IANA ID', 'Registrar Abuse Contact Email', 'Registrar Abuse Contact Phone', 'Referral URL', 'Registrar-Reseller Name', 'Reseller', 'Reseller URL', 'Reseller Name', 'Reseller Email', 'admin-contact', 'Abuse Contact', 'source', 'Sponsoring Registrar Organization', 'Sponsoring Registrar URL' ],
				},
				{
					name: 'Contact - Registrant',
					fields: [ 'Registry Registrant ID', 'Registrant ID', 'Registrant Contact ID', 'Registrant Name', 'Registrant Organization', 'Registrant Street', 'Registrant City', 'Registrant State/Province', 'Registrant Postal Code', 'Registrant Country', 'Registrant Email', 'Registrant Phone', 'Registrant Phone Ext', 'Registrant Phone Ext.', 'Registrant Fax', 'Registrant Fax Ext', 'Registrant Fax Ext.', 'Registrant Application Purpose', 'Registrant Nexus Category' ],
				},
				{
					name: 'Contact - Admin',
					fields: [ 'Registry Admin ID', 'Admin Name', 'Admin Organization', 'Admin Organisation', 'Admin Street', 'Admin City', 'Admin State/Province', 'Admin Postal Code', 'Admin Country', 'Admin Phone', 'Admin Email', 'Admin Phone Ext', 'Admin Fax', 'Admin Fax Ext', 'Admin Phone Ext.', 'Admin Fax Ext.', 'Admin Application Purpose', 'Admin Nexus Category' ],
				},
				{
					name: 'Contact - Tech',
					fields: [ 'Registry Tech ID', 'Tech Contact ID', 'Tech Name', 'Tech Contact Name', 'Tech Organization', 'Tech Organisation', 'Tech Street', 'Tech City', 'Tech State/Province', 'Tech Postal Code', 'Tech Country', 'Tech Phone', 'Tech Email', 'Tech Phone Ext', 'Tech Fax', 'Tech Fax Ext', 'Tech Phone Ext.', 'Tech Fax Ext.', 'Tech Application Purpose', 'Tech Nexus Category' ],
				},
				{
					name: 'Contact - Billing',
					fields: [ 'Registry Billing ID', 'Billing Name', 'Billing Organization', 'Billing Organisation', 'Billing Street', 'Billing City', 'Billing State/Province', 'Billing Postal Code', 'Billing Country', 'Billing Phone', 'Billing Email', 'Billing Phone Ext', 'Billing Fax', 'Billing Fax Ext', 'Billing Phone Ext.', 'Billing Fax Ext.' ],
				},
			],
	...
	<div class="p-3 pb-5 bg-slate-100 dark:bg-slate-800 flex items-center justify-between">
		<h1 class="text-3xl font-medium dark:text-white">
			<a v-if="tabType === 'domain'" :href="`https://dmns.app/${domain}?ref=browser-extension`" class="text-dark" target="_blank">
				<span class="text-neutral-500">{{ tabUrl?.hostname.replace(domain, '') }}</span>{{ domain }}
			</a>
		...
		<template v-if="tabType === 'domain'">
			<div v-if="opens > 50 && opens < 200 && !promos.includes('review')" class="bg-amber-50 rounded-md border border-amber-100 gap-3 p-2 ml-5 flex items-center">
				<p><strong class="text-slate-800">🤩</strong> Is this extension helpful? <a href="https://chrome.google.com/webstore/detail/domain-info/afbepfhknfficaflckmgflbmklcleidl/reviews" target="_blank" class="underline text-blue-800">Give it a rating now</a></p>
			...
			<div v-else-if="opens > 200 && opens < 250 && !isAllowedIncognitoAccess && !promos.includes('icognito')" class="bg-amber-50 rounded-md border border-amber-100 gap-3 p-2 ml-5 flex items-center">
				<p><strong class="text-slate-800">Pro tip:</strong> You can get domain info in Icognito mode too, if you <a :href="`chrome://extensions/?id=${id}`" target="_blank" class="underline text-blue-800">allow access from Extension settings</a></p>
			...
			<a v-else :href="`https://dmns.app/${domain}?action=monitor&ref=browser-extension`" target="_blank" class="block rounded-full py-1 px-3 bg-purple-700 hover:bg-purple-600 text-white dark:bg-indigo-800 hover:dark:bg-indigo-700 dark:text-gray-200">Monitor for changes</a>
		</template>
	</div>

	<template v-if="tabType === 'domain'">
		<div class="flex text-center">
			<div class="flex-1 py-3 cursor-pointer dark:text-white" :class="view === 'overview' ? 'font-bold' : 'bg-slate-100 hover:bg-indigo-100 dark:bg-slate-800 dark:hover:bg-indigo-900 font-medium'" @click="setView('overview')">
				Overview
			</div>
			<div class="flex-1 py-3 cursor-pointer dark:text-white" :class="view === 'history' ? 'font-bold' : 'bg-slate-100 hover:bg-indigo-100 dark:bg-slate-800 dark:hover:bg-indigo-900 font-medium'" @click="setView('history')">
				History <span v-if="Array.isArray(history)" class="bg-indigo-100 dark:bg-indigo-900 p-1 ml-1 rounded" :class="history.length ? 'text-indigo-900 dark:text-indigo-100' : 'text-neutral-700 dark:text-neutral-300'">{{ history.length }}</span>
			</div>
			<div class="flex-1 py-3 cursor-pointer dark:text-white" :class="view === 'whois' ? 'font-bold' : 'bg-slate-100 hover:bg-indigo-100 dark:bg-slate-800 dark:hover:bg-indigo-900 font-medium'" @click="setView('whois')">
				WHOIS
			</div>
			<div class="flex-1 py-3 cursor-pointer dark:text-white" :class="view === 'dns' ? 'font-bold' : 'bg-slate-100 hover:bg-indigo-100 dark:bg-slate-800 dark:hover:bg-indigo-900 font-medium'" @click="setView('dns')">
				DNS Records <span v-if="Array.isArray(records)" class="bg-indigo-100 dark:bg-indigo-900 p-1 ml-1 rounded" :class="records.length ? 'text-indigo-900 dark:text-indigo-100' : 'text-neutral-700 dark:text-neutral-300'">{{ records.length }}</span>
			</div>
		...
		<template v-if="view === 'overview'">
			...
			<template v-if="domainInfo.availability === 'registered'">
				<div class="p-3 pb-1">
					...
					<div class="grid grid-cols-3 gap-3 mb-4">
						...
						<div class="bg-blue-50 dark:bg-blue-950 hover:bg-blue-100 dark:hover:bg-blue-900 dark:text-gray-200 p-3 text-center rounded-md cursor-pointer" @click="setView('history')">
					...
					<div class="flex items-center gap-3 mb-1">
						...
						<div class="text-neutral-600 dark:text-neutral-400">
							<span v-for="ctype in contactTypes" class="capitalize cursor-pointer mx-1" :class="{'font-semibold text-slate-900 dark:text-slate-100': ctype === contactType}" @click="contactType = ctype">{{ ctype }}</span>
						</div>
					</div>

					<div v-if="domainInfo.contacts" class="flex items-center bg-slate-50 dark:bg-slate-800 gap-2 rounded-md mb-4">
						<img
							:src="getPicture(domainInfo.contacts[contactType].email || '')" :alt="domain"
							@error="$event.target.src = tabFavicon || 'https://files.layered.market/neutral-2.png'"
							class="block m-2 ml-3" width="50" height="50"
						/>
						<div class="flex-grow py-3 dark:text-gray-200">
							<p v-if="domainInfo.contacts[contactType].name || domainInfo.contacts[contactType].organization" class="text-lg mb-1">{{ uniq([domainInfo.contacts[contactType].name, domainInfo.contacts[contactType].organization].filter(Boolean)).join(', ') }}</p>
							<p v-else class="text-neutral-500 dark:text-neutral-400 mb-1">Unknown</p>

							<p v-if="domainInfo.contacts[contactType].city || domainInfo.contacts[contactType].state || domainInfo.contacts[contactType].country" class="mt-1">📍 {{ uniq([domainInfo.contacts[contactType].city, [domainInfo.contacts[contactType].state, domainInfo.contacts[contactType].postalCode].filter(Boolean).join(' '), domainInfo.contacts[contactType].country].filter(Boolean)).join(', ') }}</p>
						...
						<img v-if="domainInfo.contacts[contactType].country" :src="mapUrlForContact(domainInfo.contacts[contactType])" width="180" height="118" alt="Registrant location" class="rounded-r">
					</div>
					<div v-else class="flex items-center bg-slate-50 dark:bg-slate-800 gap-2 rounded-md mb-4">
						<img
							:src="tabFavicon || 'https://files.layered.market/neutral-2.png'" :alt="domain"
					...
					<tbody>
							...
							<td class="p-2 dark:text-gray-200">
								<Popper v-if="domainInfo.registrar" :hover="true" placement="top">
									<a v-if="domainInfo.registrar.url && domainInfo.registrar.url !== 'http://'" :href="domainInfo.registrar.url" target="_blank">
										<img :src="`https://logo.clearbit.com/${urlPart(domainInfo.registrar.url, 'hostname')}?size=32`" @error="$event.target.style.display = 'none'" height="16" class="float-left h-4 mr-1 rounded" :alt="domainInfo.registrar.name"> {{ domainInfo.registrar.name }}
									</a>
									<template v-else>{{ domainInfo.registrar.name }}</template>

									<template #content>
										<template v-for="(value, label) in domainInfo.registrar">
											<p v-if="label !== 'name'" class="mb-1 last:mb-0"><span class="uppercase">{{ label }}</span>: <strong>{{ value }}</strong></p>
										</template>
									</template>
								</Popper>
								<span v-else>..</span>
						...
						<tr class="hover:bg-slate-100 dark:hover:bg-slate-900 border-b border-slate-100 dark:border-slate-900">
							...
							<td class="py-1 px-2 dark:text-gray-200">
								<span v-for="status in domainInfo.status" class="inline-block bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-300 rounded px-1 my-1 mr-2">{{ status }}</span>
							</td>
						</tr>
						<tr class="hover:bg-slate-100 dark:hover:bg-slate-900 border-b border-slate-100 dark:border-slate-900">
							<td class="p-2 pl-3 dark:text-gray-300">Name Servers</td>
							<td class="py-1 px-2 dark:text-gray-200">
								<span v-for="ns in nameServers" class="inline-block bg-stone-100 text-stone-600 dark:bg-stone-800 dark:text-stone-300 rounded px-1 my-1 mr-2">{{ ns }}</span>
							</td>
						</tr>
						...
						<tr v-for="service in services" class="hover:bg-slate-100 dark:hover:bg-slate-900 border-b border-slate-100 dark:border-slate-900">
							...
							<td class="py-1 px-2 dark:text-gray-200">
								<a :href="service.url" target="_blank" :title="service.url">
									<img :src="service.logo" @error="$event.target.style.display = 'none'" height="16" class="float-left h-4 mr-1 rounded" :alt="service.name"> {{ service.name }}
								</a>
							</td>
						</tr>
					</tbody>
				...
				<template v-if="showRelated">
					<div class="p-3">
						<div class="flex items-center gap-3 mb-1">
							...
							<div class="text-neutral-600 dark:text-neutral-400">
								<span class="capitalize cursor-pointer mx-1" :class="{'font-semibold text-slate-900 dark:text-slate-100': related === 'tlds'}" @click="related = 'tlds'">Other TLDs</span>
								<span class="capitalize cursor-pointer mx-1" :class="{'font-semibold text-slate-900 dark:text-slate-100': related === 'similar-websites'}" @click="related = 'similar-websites'">Similar websites ({{ relatedSimilarWebsites.length }})</span>
							</div>
						</div>

						<div v-if="related === 'tlds'">
							[-]
						</div>
						<div v-else-if="related === 'similar-websites'">
							<a v-for="website in relatedSimilarWebsites" :href="`https://dmns.app/${website.domain}`" target="_blank" class="inline-block px-1 rounded bg-slate-100 text-slate-900 mr-2 mb-2">
								{{ website.domain }}
			...
			<template v-else-if="domainInfo.availability === 'available'">
				<div class="m-9 bg-cyan-50 dark:bg-cyan-950 border border-cyan-100 dark:border-cyan-800 rounded-md px-3 py-4 text-center dark:text-gray-200">
					...
					<p>
						<a href="https://porkbun.com/checkout/search?ref=andreiigna&q=adamadamadamadam.com" target="_blank" class="inline-block px-2 py-1 rounded bg-slate-50 border border-slate-200 hover:border-slate-300 mx-1"><img class="float-left me-1" src="https://porkbun.com/favicon.ico" alt="Porkbun" width="16" height="16"> Porkbun</a>
						<a href="https://www.dynadot.com/domain/search?domain=adamadamadamadam.com" target="_blank" class="inline-block px-2 py-1 rounded bg-slate-50 border border-slate-200 hover:border-slate-300 mx-1"><img class="float-left me-1" src="https://www.dynadot.com/favicon.ico" alt="Dynadot" width="16" height="16"> Dynadot</a>
						<a href="https://www.namecheap.com/domains/registration/results/?domain=adamadamadamadam.com" target="_blank" class="inline-block px-2 py-1 rounded bg-slate-50 border border-slate-200 hover:border-slate-300 mx-1"><img class="float-left me-1" src="https://www.namecheap.com/favicon.ico" alt="Namecheap" width="16" height="16"> Namecheap</a>
					</p>
				</div>
			</template>
			...
			<template v-else-if="domainInfo.availability === 'reserved'">
				<div class="m-6 bg-purple-50 border border-purple-100 rounded-md p-3 flex gap-x-4 items-center">
					<div class="flex-grow text-center">
						...
						<p>This domain can't be registered. <a href="https://en.wikipedia.org/wiki/Special-use_domain_name" target="_blank" class="underline">Learn more here</a></p>
					</div>
				</div>
			</template>
		...
		<template v-else-if="view === 'whois'">
			...
			<div v-if="states.whois === 'loading'" class="p-3 text-center dark:text-gray-300">
				Hang on tight, we're looking up the WHOIS info for this domain.
			</div>
			<div v-else-if="states.whois === 'error'" class="p-3 text-center dark:text-gray-300">
				⚠️ There was an error loading the WHOIS info for this domain.
			</div>
			<template v-else>
				<div class="p-3 dark:text-gray-300">
					<p v-if="whoisServers.length > 1" class="leading-5 mb-4">We found WHOIS info in Registry <code class="rounded p-1 bg-cyan-50 text-cyan-900 dark:bg-cyan-950 dark:text-cyan-300">{{ whoisServers[0] }}</code> and Registrar <code class="rounded p-1 bg-sky-50 text-sky-900 dark:bg-sky-950 dark:text-sky-300">{{ whoisServers[1] }}</code> servers. Data from Registrar is usually more complete, and the differences between them are highlighted below.</p>
					<p v-else class="leading-5 mb-4">This is the WHOIS data we found at the Registry WHOIS server <code class="rounded p-1 bg-cyan-50 text-cyan-900 dark:bg-cyan-950 dark:text-cyan-300">{{ whoisServers[0] }}</code></p>

					<div class="flex items-center justify-endd">
						<div class="text-slate-600 dark:text-slate-300 pr-1">
							Data format:
						</div>
						<div class="bg-slate-50 dark:bg-slate-900 p-1 rounded-full">
							<button class="rounded-full py-1 px-2" :class="whoisView === 'table' ? 'bg-indigo-200 dark:bg-indigo-800' : 'hover:bg-indigo-50 dark:hover:bg-indigo-900'" @click="whoisView = 'table'">Table</button>
							<button class="rounded-full py-1 px-2 ml-1" :class="whoisView === 'json' ? 'bg-indigo-200 dark:bg-indigo-800' : 'hover:bg-indigo-50 dark:hover:bg-indigo-900'" @click="whoisView = 'json'">JSON</button>
						...
						<template v-if="whoisView === 'table' && whoisServers.length > 1">
							<div class="text-slate-600 dark:text-slate-300 pl-4 pr-1">
								Data from:
							</div>
							<div class="bg-slate-50 dark:bg-slate-900 p-1 rounded-full">
								<button class="rounded-full py-1 px-2" :class="whoisData === 'registry' ? 'bg-violet-200 dark:bg-violet-800' : 'hover:bg-violet-50 dark:hover:bg-violet-900'" @click="whoisData = 'registry'">Registry</button>
								<button class="rounded-full py-1 px-2 mx-1" :class="whoisData === 'registrar' ? 'bg-violet-200 dark:bg-violet-800' : 'hover:bg-violet-50 dark:hover:bg-violet-900'" @click="whoisData = 'registrar'">Registrar</button>
								<button class="rounded-full py-1 px-2" :class="whoisData === 'both' ? 'bg-violet-200 dark:bg-violet-800' : 'hover:bg-violet-50 dark:hover:bg-violet-900'" @click="whoisData = 'both'">Both</button>
							</div>
						</template>

						<label v-if="whoisView === 'table' && whoisServers.length > 1 && whoisData === 'both'" class="rounded-md py-1 px-2 ml-4 flex" :class="whoisDiff ? 'bg-yellow-50 dark:bg-yellow-900' : 'bg-slate-50 dark:bg-slate-900'">
				...
				<table v-if="whoisView === 'table'" class="w-full">
					<template v-for="group in whoisGroup">
						<thead>
							<tr class="bg-stone-100 dark:bg-neutral-900 mt-2">
								...
								<th v-if="whoisServers.length > 1 && ['registrar', 'both'].includes(whoisData)" class="px-1 py-1 text-left text-neutral-500 dark:text-neutral-400">Registrar data</th>
							</tr>
						</thead>
						<tbody>
							<template v-for="field in group.fields">
								<tr v-if="whois[whoisServers[0]][field] || whois[whoisServers[1]]?.[field]" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900" :class="{'bg-amber-50 hover:bg-amber-100 dark:bg-yellow-950 hover:dark:bg-yellow-900': whoisDataDifferent(field)}">
					...
					<tbody>
						<template v-for="(value, field) in (whois[whoisServers[1]] || whois[whoisServers[0]])">
							<tr v-if="!whoisGroupFields.includes(field)" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900" :class="{'bg-amber-50 hover:bg-amber-100 dark:bg-yellow-950 hover:dark:bg-yellow-900': whoisDataDifferent(field)}">
								<td class="pl-2 pr-1 py-2">{{ field }}</td>
								<td v-if="['registry', 'both'].includes(whoisData)" class="px-1 py-2">
									<template v-if="Array.isArray(whois[whoisServers[0]][field])">
										<p v-for="line in whois[whoisServers[0]][field]">{{ field === 'Domain Status' ? line.split(' ')[0] : line }}</p>
				...
				<div class="bg-orange-50 dark:bg-amber-950 p-3 my-4 mx-3 rounded-md">
					...
					<p class="text-sm mb-2 dark:text-slate-300">Do you want this data in JSON format for your project? Try out the <a href="https://docs.dmns.app/api" target="_blank" class="underline">Domains API</a></p>
		...
		<template v-else-if="view === 'history'">
			...
			<div v-if="states.history === 'loading'" class="p-3 text-center dark:text-gray-200">
				Hang on tight, we're looking up the history for this domain.
			</div>
			<div v-else-if="states.history === 'error'" class="p-3 text-center dark:text-gray-200">
				⚠️ There was an error loading the history for this domain.
			</div>
			<div v-else class="p-3">
				<p v-if="history.length" class="mb-3 dark:text-gray-200">These are the changes we detected for this domain. <a :href="`https://dmns.app/${domain}?action=monitor&ref=browser-extension`" target="_blank" class="underline text-blue-800 dark:text-blue-300">Monitor the domain</a> to get instant alerts when more changes are detected.</p>
				<div v-else class="m-6 bg-indigo-50 dark:bg-indigo-950 border border-indigo-100 dark:border-indigo-800 rounded-md p-3 text-center dark:text-gray-200">
					...
					<p><a :href="`https://dmns.app/${domain}?action=monitor&ref=browser-extension`" target="_blank" class="underline text-blue-800 dark:text-blue-300">Monitor the domain</a> to get instant alerts when a change is detected <a href="https://dmns.app/monitor-domains" target="_blank" class="inline-block ml-2">ℹ️</a></p>
				</div>
				...
				<div v-for="activity in history" class="border-l-2 border-neutral-300 dark:border-neutral-600 ml-3 py-3 pl-6 relative">
					...
					<div v-else-if="activity.type === 'sale'">
						<div class="absolute bg-emerald-100 dark:bg-emerald-700 rounded-full text-lg leading-none p-2 mt-2" style="left: -17px">
							💸
						</div>
						...
						<p class="px-3 text-neutral-800 dark:text-neutral-400">
							<DateTime :date="new Date(activity.created_at)" :style="whoisTimeStyle" @click="switchWhoisTimeStyle"></DateTime>
							&middot;
							Source: <a v-if="activity.data.source === 'namebio'" :href="`https://namebio.com/${domain}`" target="_blank">Namebio.com</a><template v-else>{{ activity.data.source }}</template>
						</p>
					</div>
					<div v-else-if="activity.type === 'url'">
						...
						<p class="px-3 text-neutral-800 dark:text-neutral-400">
							<DateTime :date="new Date(activity.created_at)" :style="whoisTimeStyle" @click="switchWhoisTimeStyle"></DateTime>
							&middot;
							Source: {{ urlPart(activity.data.url, 'hostname') }}
						</p>
					</div>
					<div v-else-if="activity.type === 'note'">
						<div class="absolute bg-amber-100 dark:bg-amber-700 rounded-full text-lg leading-none p-2 mt-1" style="left: -17px">
							📝
						</div>
				...
				<div v-if="history.length && domainInfo.dates?.created" class="border-l-2 border-neutral-300 dark:border-neutral-600 ml-3 pt-3 pl-6 relative">
					<div class="absolute bg-neutral-100 dark:bg-neutral-700 rounded-full text-lg leading-none p-2 mt-1" style="left: -17px">
						🔘
					</div>
		...
		<template v-else-if="view === 'dns'">
			...
			<div v-if="states.dns === 'error'" class="p-3 text-center">
				⚠️ There was an error loading the history for this domain.
			</div>

			<div v-else-if="domainInfo.availability === 'available'" class="m-6 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 dark:text-gray-200 rounded-md p-3 flex gap-x-4 items-center">
				<img src="https://dmns.app/images/plan-pro.png" class="flex-none" width="100" height="100">
			...
			<div v-else-if="states.dns === 'loaded' && !records.length" class="m-6 bg-indigo-50 border border-indigo-100 rounded-md p-3 flex gap-x-4 items-center">
				...
				<div class="flex-grow">
					...
					<p class=""><a :href="`https://dmns.app/${domain}?action=monitor&ref=browser-extension`" target="_blank" class="underline text-blue-800 dark:text-blue-300">Monitor the domain</a> to get instant alerts when a change is detected</p>
				</div>
			</div>
			...
			<template v-else>
				<div class="p-3 flex items-center">
					<div class="flex-grow">
						<p class="dark:text-gray-50">
							...
							<img v-if="states.dns === 'loading'" src="/three-dots.svg" class="inline bg-slate-300 dark:bg-slate-900 rounded ms-1 px-2 py-1" width="48" alt="Loader" />
						</p>
					</div>
					<a :href="`https://dmns.app/${domain}/dns-records?action=download-zone-file`" target="_blank" class="block rounded-full py-1 px-3 bg-indigo-200 hover:bg-indigo-300 text-gray-800 dark:bg-gray-800 hover:dark:bg-gray-700 dark:text-gray-200">Download zone file</a>
				</div>
				...
				<table class="w-full mb-3">
					...
					<tbody>
						<tr v-if="records.find(r => r.type === 'NS')" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
						...
						<tr v-for="record in records.filter(r => ['CAA', 'SOA'].includes(r.type))" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
							<td class="py-2 pl-2 pr-1">
					...
					<tbody>
						<tr v-for="record in recordsGrouped.A" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
							...
							<td class="py-2 pl-1 pr-2">
								<p v-for="value in record.data" class="break-all mb-1 last:mb-0">
									<Popper :hover="true" placement="top" @open:popper="loadIpInfo(value)">
										<span class="inline-block rounded px-1 hover:bg-neutral-200 dark:hover:bg-gray-700" @click="$event => copyText($event, value)">{{ value }} <span class="text-blue-500">ℹ</span></span>

										<template #content>
											<h6 class="mb-2">IP Info for <strong>{{ value }}</strong></h6>
											...
											<template v-else>
												<p class="mb-1">Org: <strong>{{ ipInfo[value].org }}</strong></p>
												<p class="mb-1 last:mb-0">Location: <strong>{{ [ipInfo[value].city, ipInfo[value].region, ipInfo[value].country].filter(Boolean).join(', ') }}</strong></p>
												<p v-if="ipInfo[value].hostname" class="mb-1 last:mb-0">Hostname: <strong>{{ ipInfo[value].hostname }}</strong></p>
											</template>
										</template>
									</Popper>
								</p>
						...
						<tr v-for="record in recordsGrouped.AAAA" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
							<td class="py-2 pl-2 pr-1 align-top">
						...
						<tr v-for="record in recordsGrouped.CNAME" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
							<td class="py-2 pl-2 pr-1 align-top">
					...
					<tbody>
						<tr v-if="records.find(r => r.type === 'MX')" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
							<td class="py-2 pl-2 pr-1 align-top">
					...
					<tbody>
						<tr v-for="record in recordsGrouped.TXT" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
							<td class="py-2 pl-2 pr-1 align-top">
						...
						<tr v-for="record in records.filter(r => r.type && !['NS', 'SOA', 'CAA', 'MX', 'A', 'AAAA', 'CNAME', 'TXT'].includes(r.type))" class="border-b border-slate-100 hover:bg-neutral-50 dark:text-neutral-200 dark:hover:bg-gray-900 dark:border-slate-900">
							<td class="py-2 pl-2 pr-1">
								<span class="inline-block font-medium px-1 rounded bg-stone-200 text-stone-600 dark:bg-stone-700 dark:text-stone-400">{{ record.type }}</span>
							</td>
							<td class="py-2 px-1">{{ record.name }}</td>
