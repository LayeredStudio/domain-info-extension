<template>
	<div class="app-popup">
		...
		<h2 v-if="domainRoot" class="text-center"><span class="text-muted">{{ domain.replace(domainRoot, '') }}</span>{{ domainRoot }}</h2>
						...
						<table class="table table-sm table-hover">
							...
							<tbody v-for="(group, type) in data.dns">
								<tr v-for="dns in group">
									<td><span class="badge" :class="{'badge-secondary': ['NS', 'SOA'].includes(dns.type), 'badge-success': ['A', 'AAAA', 'CNAME'].includes(dns.type), 'badge-primary': ['MX', 'TXT'].includes(dns.type), 'badge-info': !['NS', 'SOA', 'A', 'AAAA', 'CNAME', 'MX', 'TXT'].includes(dns.type)}">{{ dns.type }}</span></td>
									<td><span :class="{ 'badge badge-light': isWildcardSubdomain(dns.name) }">{{ dns.name }}</span></td>
									<td><span class="ttl">{{ toTime(dns.ttl) }}</span></td>
									<td>
					...
					<div v-else-if="tab.title === 'Overview'" class="box">
						...
						<div class="row mt-4 mb-3">
							<div class="col-auto text-center">
								...
								<span v-if="tab.content.status === 'registered' && tab.content.registrar.name" class="text-muted">
									at
									<a v-if="tab.content.registrar.url" target="_blank" :href="(tab.content.registrar.url.includes('://') ? '' : 'http://') + tab.content.registrar.url">{{ tab.content.registrar.name }}</a>
						...
						<div v-if="tab.content.status === 'registered'" class="bg-light rounded p-3 mb-3">
							<div class="row align-items-center text-center">
								<div class="col">
									<p class="mb-2 lead"><strong>{{ formatDate(tab.content.dates.created) }}</strong></p>
									<p class="text-uppercase text-muted mb-0">Created</p>
								</div>
								<div class="col">
									<p class="mb-2 lead"><strong>{{ formatDate(tab.content.dates.updated) }}</strong></p>
									<p class="text-uppercase text-muted mb-0">Updated</p>
								</div>
								<div class="col">
									<p class="mb-2 lead"><strong>{{ formatDate(tab.content.dates.expiry) }}</strong></p>
						...
						<div v-if="tab.content.status === 'registered' && data.whoisD" class="bg-light rounded p-3 mb-3">
							<div class="row align-items-center">
								<div class="col-auto">
									<img :src="getPicture(data.whoisD['Registrant Email'] || '')" @error="$event.target.src = 'https://files.layered.market/neutral-2.png'" width="50" class="rounded" alt="Registrant" />
								</div>
								<div class="col">
									<p class="mb-1 lead">{{ Array.from(new Set([data.whoisD['Registrant Name'], data.whoisD['Registrant Organization'], data.whoisD['Registrant Organisation']].filter(Boolean))).join(', ') || 'Anonymous' }}</p>
									<p class="mb-1">{{ Array.from(new Set([data.whoisD['Registrant City'], data.whoisD['Registrant State/Province'], data.whoisD['Registrant State'], data.whoisD['Registrant Country']].filter(Boolean))).join(', ') }}</p>
						...
						<div v-if="tab.content.status === 'available'" class="bg-light rounded p-3 mb-3">
							...
							<p>Register now with <a :href="'https://domains.google.com/m/registrar/search?searchTerm=' + (domainRoot || domain)" target="_blank">Google Domains</a>, <a :href="'https://porkbun.com/checkout/search?q=' + (domainRoot || domain)" target="_blank">PorkBun</a> or <a :href="'https://www.godaddy.com/domainsearch/find?domainToCheck=' + (domainRoot || domain)" target="_blank">GoDaddy</a>.</p>
						</div>
...
export default {
	data() {
		return {
			...
			whoisGroup:	[{
				title:	'Domain Info',
				fields:	['Domain Name', 'Domain Status', 'Name Server', 'Domain nameservers', 'ROID', 'Registry Domain ID', 'IDN', 'Whoisprivacy', 'Created Date', 'Updated Date', 'Expiry Date', 'free-date', 'Registration status', 'DNSSEC', 'DNSSEC DS Data', 'Error', 'ERROR'],
				found:	0
			}, {
				title:	'Registrar',
				fields:	['Registrar', 'Sponsoring Registrar', 'Record maintained by', 'Registrar URL', 'Registrar WHOIS Server', 'Registry WHOIS Server', 'Registrar IANA ID', 'Registrar Abuse Contact Email', 'Abuse Contact', 'Registrar Abuse Contact Phone', 'Reseller', 'Reseller URL', 'Referral URL', 'Registrar-Reseller Name', 'Reseller Email', 'admin-contact', 'source'],
				found:	0
			}, {
				title:	'Contact - Registrant',
				fields:	['Registry Registrant ID', 'Registrant ID', 'Registrant Name', 'Registrant Organization', 'Registrant Organisation', 'Registrant Street', 'Registrant City', 'Registrant State/Province', 'Registrant State', 'Registrant Postal Code', 'Registrant Country', 'Registrant Phone', 'Registrant Email', 'Registrant Phone Ext', 'Registrant Fax', 'Registrant Fax Ext', 'Registrant Phone Ext.', 'Registrant Fax Ext.', 'Registrant Application Purpose', 'Registrant Nexus Category'],
				found:	0
			}, {
				title:	'Contact - Admin',
				...
				title:	'Contact - Tech',
				fields:	['Registry Tech ID', 'Tech Name', 'Tech Organization', 'Tech Organisation', 'Tech Street', 'Tech City', 'Tech State/Province', 'Tech Postal Code', 'Tech Country', 'Tech Phone', 'Tech Email', 'Tech Phone Ext', 'Tech Fax', 'Tech Fax Ext', 'Tech Phone Ext.', 'Tech Fax Ext.', 'Tech Application Purpose', 'Tech Nexus Category'],
				found:	0
			}, {
...
.tabs ul {
	...
	li {
		...
		border-radius: 2px;
		border-bottom: 2px solid rgba(70,13,141,.2);
		cursor: pointer;
		text-align: center;
	}
